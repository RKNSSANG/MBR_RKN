...
class Node:
    def __init__(self):
        ...
        if random.random() < 0.5:
            self.state = 'C'
            self.states.append('C')     #状态初始化
            self.states.extend(['C','D','C','D'])   #为MBR选择添加4个状态
        else:
            self.state = 'D'
            self.states.append('D')
            self.states.extend(['C', 'D', 'C', 'D'])
        self.value = 0
        self.all_value = 0
    ...
    def updateState(self):
        ...
        for i in range(self.numbers):
            if self.nodes[i].state == 'C':
                ...
                if reward2 <= reward1:
                    del(self.nodes[i].states[0])
                    self.nodes[i].states.append('C')

                    self.nodes[i].state = choice(self.nodes[i].states)
                    continue
                elif reward2 >= reward1:
                    del (self.nodes[i].states[0])
                    self.nodes[i].states.append('D')
                    self.nodes[i].state = choice(self.nodes[i].states)
                    continue
                flag = True
...
if __name__ == '__main__':
    ...
    net.buildNet(({net.nodes[a], net.nodes[b]} for a, b in edge_list))

    # 循环更新节点状态，直至不愿改变自己的决策
